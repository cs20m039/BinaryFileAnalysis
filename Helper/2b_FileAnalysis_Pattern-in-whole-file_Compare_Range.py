...
LOG_FILE_PATH = f'logfiles/log-analyse-header_{timestamp}.txt'
...
DIRECTORY_PATH = '/home/cs20m039/thesis/dataset/benign/System'
                ...
                with open(full_path, 'rb') as file:
                    ...
                    for pattern_key in hex_data_by_length.get(length, []):
                        ...
                        if pattern_bytes in file_content:
                            ...
                            logging.debug(f'Match found: Pattern "{hex_value}" from "{source_file}" in file "{filename}"')
                            break  # Assuming you only need the first match for each file

        if current_match_count != previous_match_count:
            logging.info(f"Length {length} bytes: {current_match_count} files")  # Directly print to console
