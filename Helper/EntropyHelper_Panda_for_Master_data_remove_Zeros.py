...
# Function to replace zeros with the next non-zero value
...
for column in columns_to_modify:
    if 'Percentage' not in column:  # Assuming percentage columns should remain float
        # Convert to int if the column was originally int
        merged_df[column] = merged_df[column].astype(int)
    else:
        # Ensure percentage columns are rounded to 2 decimal places if they are not integers
        merged_df[column] = merged_df[column].round(2)

# Display the modified DataFrame (or save to CSV/Excel)
