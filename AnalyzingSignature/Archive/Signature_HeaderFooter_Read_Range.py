    ...
    try:
        with open(file_path, 'rb') as file:
            ...
            for length in range(INTERVAL_START, INTERVAL_END + 1):
                ...
                last_bytes = last_bytes_read[-length:].hex() if len(
                    last_bytes_read) >= length else last_bytes_read.hex()
...
def analyze_files_recursive(directory_path, csv_path):
    ...
    with open(csv_path, 'w', newline='') as csv_file:
        ...
        headers = ['FileHash'] + [f'{i}ByteFirst' for i in range(INTERVAL_START, INTERVAL_END + 1)] + [f'{i}ByteLast'
                                                                                                       for i in range(
            ...
            if filenames:
                ...
                for filename in filenames:
                    ...
                    if bytes_data:
                        ...
                        row = [hash_value] + [bytes_data.get(f'{i}ByteFirst', '') for i in
                                              range(INTERVAL_START, INTERVAL_END + 1)] + [
                                  bytes_data.get(f'{i}ByteLast', '') for i in range(INTERVAL_START, INTERVAL_END + 1)]
                        writer.writerow(row)
                        file_count += 1
            else:
