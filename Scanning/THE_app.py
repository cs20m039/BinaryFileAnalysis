...
def compare_entropy(directory, patterns):
    ...
    files_scanned = 0
    files_processed = 0  # Counter for files that are actually processed
    for root, dirs, files in os.walk(directory, followlinks=False):
        ...
        for file in files:
            ...
            try:
                ...
                files_processed += 1  # Increment here since the file meets the size criterion
    ...
    logger.info(
        f"Scanning completed. Files scanned: {files_scanned}, Files processed (>=500 bytes): {files_processed}, Matches found: {len(matches['benign'])} benign, {len(matches['malware'])} malware, and {len(matches['unknown'])} unknown. Non-matching files: {matches['non_matching']}.")
...
def main():
    ...
    for config in entropy_scan_configs:
        ...
        files_scanned, files_processed, matches = compare_entropy(directory_to_scan, entropy_patterns)  # Corrected this line
